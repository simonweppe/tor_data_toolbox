...
function [figDescrip13] = plot_13(matlabfile_processed,all,timeframe,label2,label3) %Zeitachse wird nicht modifiziert (wie in den anderen xy-Plots)
% load('SCT_DATA_FRA56.mat');
load(matlabfile_processed)
figDescrip13 = "Vessel Speed [kn]"; 
figDescrip13 = char(figDescrip13); %beschreibt das Figure und nutzt den Char für den Dokumentennamen

fig13_handle = figure(13);
...
Minrange = floor(min(Speed));
Maxrange = ceil(max(Speed));
plotbin(x,y,Speed,[Minrange:(Maxrange-Minrange)/10:Maxrange],20);
% plotbin(x,y,Speed,[0:2:20],20);

% title(figDescrip13); %Kurztitel
combinedStr     = strcat(figDescrip13, {', '}, {'TJV19'}); %für den Titel der Abb musste das Format angepasst werden.
title(combinedStr);
% title({SCTlabel1, figDescrip13}); %Titel mit mehreren Zeilen
