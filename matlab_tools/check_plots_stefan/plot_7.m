...
function [figDescrip7] = plot_7(matlabfile_processed,all,timeframe)
% load('SCT_DATA_FRA56.mat'); 
load(matlabfile_processed)
figDescrip7 = "GPS"; figDescrip7 = char(figDescrip7); %beschreibt das Figure und nutzt den Char für den Dokumentennamen

fig07_handle = figure(7);
fig07_handle.Position = [0 0 1600 900];
...
plot1_handle = plot(ax1_handle, dt, Latitude,    '.','color','black');        % [0, 0, 0]
plot2_handle = plot(ax2_handle, dt, Longitude,    '.','color','red');          % [1, 0, 0]
plot3_handle = plot(ax3_handle, dt, Speed,  '.','color','blue');
plot4_handle = plot(ax4_handle, dt, Course,      '.','color', [0, 0.5 , 0]);  % dark green
plot5_handle = plot(ax5_handle, dt, MagnVar,  '.','color', [0, 1 , 1]);  % cyan

% Y-Label
ax1_handle.YLabel.String = ({'Latitude', '[°N]'});
ax2_handle.YLabel.String = ({'Longitude', '[°E]'});
ax3_handle.YLabel.String = ({'Vessel speed', '[kn]'});
ax4_handle.YLabel.String = ({'Course', '[°]'});
ax5_handle.YLabel.String = ('Magn.Var');
...
% set(ax5_handle,'yticklabel',{'Pump off','Pump on'});
