%% Figure 04: MK2 valves

function [figDescrip4] = plot_4(matlabfile_processed,all,timeframe)
% load('SCT_DATA_FRA56.mat'); 
load(matlabfile_processed)
figDescrip4 = "MK2_valves"; figDescrip4 = char(figDescrip4); %beschreibt das Figure und nutzt den Char für den Dokumentennamen

fig04_handle = figure(4);
fig04_handle.Position = [0 0 1600 900];
...
plot1_handle = plot(ax1_handle, dt, VALVE1,    '.','color','black');        % [0, 0, 0]
plot2_handle = plot(ax2_handle, dt, VALVE2,    '.','color','red');          % [1, 0, 0]
plot3_handle = plot(ax3_handle, dt, VALVE3,  '.','color','blue');
plot4_handle = plot(ax4_handle, dt, VALVE4,      '.','color', [0, 0.5 , 0]);  % dark green
plot5_handle = plot(ax5_handle, dt, STATUS,  '.','color', [0, 1 , 1]);  % cyan

% Y-Label
ax1_handle.YLabel.String = ({'Valve1' ,'(ZERO out)'});
ax2_handle.YLabel.String = ({'Valve2' ,'(ZERO in)'});
ax3_handle.YLabel.String = ({'Valve3' ,'(Gas out)'});
ax4_handle.YLabel.String = ({'Valve4' ,'(Gas in)'});
